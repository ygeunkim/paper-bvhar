---
title: "Modeling Simulation Data"
subtitle: "`r params$size`-`r params$simulation` data"
author: "Young Geun Kim"
date: "`r format(Sys.time(), '%d %b, %Y')`"
output: 
  bookdown::html_document2:
    toc: true
  bookdown::pdf_document2:
    toc: true
params:
  simulation:
    label: "Simulation model:"
    value: vhar
    choices: [vhar, var]
  size:
    label: "Simulation size:"
    value: medium
    choices: [small, medium, large]
  h:
    label: "Out of sample:"
    value: 5
    input: slider
    min: 1
    max: 100
    step: 1
  var_lag:
    label: "Oder of VAR:"
    value: 3
    input: slider
    min: 1
    max: 25
    step: 1
  include_mean:
    label: "Include constant term?"
    value: FALSE
    input: checkbox
  bvar_del:
    label: "delta of BVAR:"
    value: .3
    input: numeric
  bvar_lam:
    label: "lambda of BVAR:"
    value: .1
    input: numeric
  bvar_lag:
    label: "Order of BVAR"
    value: 3
    input: slider
    min: 1
    max: 25
    step: 1
  bvhar_del:
    label: "delta of BVHAR:"
    value: .3
    input: numeric
  bvhar_lam:
    label: "lambda of BVHAR:"
    value: .1
    input: numeric
  daily_wt:
    label: "Daily weight:"
    value: .3
    input: numeric
  weekly_wt:
    label: "Weekly weight:"
    value: .1
    input: numeric
  monthly_wt:
    label: "Monthly weight:"
    value: .1
    input: numeric
  predplot_cut:
    label: "Prediction plot x label from:"
    value: 4950
    input: slider
    min: 1
    max: 5000
    step: 10
knit:
  (function(inputFile, encoding) {
    rmarkdown::render(
      input = inputFile,
      output_dir = "../docs/results", 
      params = "ask", 
      encoding = encoding
    )
  })
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  comment = "#>",
  collapse = TRUE,
  out.width = "70%",
  fig.align = "center",
  fig.width = 6,
  fig.asp = .618,
  fig.pos = "H"
)
knitr::knit_hooks$set(
  document = function(x) {
    sub("\\usepackage[]{color}", "\\usepackage{xcolor}", x, fixed = TRUE)
  }
)
options(digits = 3)
options(knitr.kable.NA = "")
options(kableExtra.latex.load_packages = FALSE)
is_html <- knitr::opts_knit$get("rmarkdown.pandoc.to") == "html"
is_latex <- knitr::opts_knit$get("rmarkdown.pandoc.to") == "latex"
kable_format <- ifelse(!is_html, "latex", "html")
kable_format <- ifelse(length(kable_format) == 0, "html", kable_format) # for excecutable
```

```{r pkgs}
library(tidyverse)
library(bvhar)
library(foreach)
library(knitr)
library(kableExtra)
```

```{r fns}
source("kable-fns.R")
```

# Dataset

## Import

```{r, message=FALSE}
# Train data set----------------------------
tr_file <- 
  str_c(
    "../data/processed/",
    params$simulation,
    "sim"
  ) %>% 
  str_c(params$size, "train.csv", sep = "_")
# Test data set----------------------------
te_file <- 
  str_c(
    "../data/processed/",
    params$simulation,
    "sim"
  ) %>% 
  str_c(params$size, "test.csv", sep = "_")
# Import-----------------------------------
y_train <- read_csv(tr_file)
y_test <- read_csv(te_file)
num_test <- params$h
y_test <- 
  y_test %>% 
  slice(1:num_test)
```

```{r, echo=FALSE}
y_train %>% 
  mutate(train = TRUE) %>% 
  bind_rows(y_test %>% mutate(train = FALSE)) %>% 
  mutate(id = 1:n()) %>% 
  pivot_longer(-c(id, train), names_to = "asset", values_to = "value") %>% 
  ggplot(aes(x = id, y = value)) +
  # geom_path(aes(colour = train), show.legend = FALSE) +
  # facet_grid(asset ~ ., scales = "free_y") +
  annotate(
    geom = "rect",
    xmin = nrow(y_train),
    xmax = Inf,
    ymin = -Inf,
    ymax = Inf,
    alpha = .7,
    fill = "grey" # test set
  ) +
  geom_path(aes(colour = asset), show.legend = FALSE, alpha = .5) +
  scale_colour_viridis_d() +
  theme_minimal() +
  labs(
    x = element_blank(),
    y = element_blank()
  )
```

# Models

## VAR

```{r}
(var_lag <- params$var_lag)
```

```{r}
(include_mean <- params$include_mean)
```

```{r}
fit_var <- var_lm(y_train, var_lag, include_mean = include_mean)
pred_var <- predict(fit_var, num_test)
(mse_var <- mse(pred_var, y_test))
```

## VHAR

```{r}
fit_vhar <- vhar_lm(y_train, include_mean = include_mean)
pred_vhar <- predict(fit_vhar, num_test)
(mse_vhar <- mse(pred_vhar, y_test))
```

## BVAR

### Minnesota prior specification

- Shrinkage hyperparameter
- delta: White noise ($\delta_i = 0$) versus random walk ($\delta_i = 1$, Litterman)
- sigma: $\Sigma_e = diag(\sigma_1, \ldots, \sigma_m)$: use sd of each variable

```{r}
(bvar_spec <- set_bvar(
  sigma = apply(y_train, 2, sd),
  lambda = params$bvar_lam,
  delta = rep(params$bvar_del, ncol(y_train)),
  eps = 1e-04
))
```

### fit

```{r}
(bvar_lag <- params$bvar_lag)
```

```{r}
fit_bvar <- bvar_minnesota(y_train, bvar_lag, bayes_spec = bvar_spec, include_mean = include_mean)
pred_bvar <- predict(fit_bvar, num_test)
(mse_bvar <- mse(pred_bvar, y_test))
```

## BVHAR

### VAR-type BVHAR

```{r}
(bvhar_spec_v1 <- set_bvhar(
  sigma = apply(y_train, 2, sd),
  lambda = params$bvhar_lam,
  delta = rep(params$bvhar_del, ncol(y_train)),
  eps = 1e-04
))
```

```{r}
fit_bvhar_v1 <- bvhar_minnesota(y_train, bayes_spec = bvhar_spec_v1, include_mean = include_mean)
pred_bvhar_v1 <- predict(fit_bvhar_v1, num_test)
(mse_bvhar_v1 <- mse(pred_bvhar_v1, y_test))
```

### VHAR-type BVHAR

$\delta_i$ for every daily, weekly, monthly matrix

```{r}
(bvhar_spec_v2 <- set_weight_bvhar(
  sigma = apply(y_train, 2, sd),
  lambda = params$bvhar_lam,
  eps = 1e-04,
  daily = rep(params$daily_wt, ncol(y_train)),
  weekly = rep(params$weekly_wt, ncol(y_train)),
  monthly = rep(params$monthly_wt, ncol(y_train))
))
```

```{r}
fit_bvhar_v2 <- bvhar_minnesota(y_train, bayes_spec = bvhar_spec_v2, include_mean = include_mean)
pred_bvhar_v2 <- predict(fit_bvhar_v2, num_test)
(mse_bvhar_v2 <- mse(pred_bvhar_v2, y_test))
```

## Compare

### Plot

```{r}
plot_cut <- params$predplot_cut
autoplot(pred_var, x_cut = plot_cut, ci_alpha = .6, type = "wrap") +
  autolayer(pred_vhar, ci_alpha = .5) +
  autolayer(pred_bvar, ci_alpha = .4) +
  autolayer(pred_bvhar_v1, ci_alpha = .3) +
  autolayer(pred_bvhar_v2, ci_alpha = .2) +
  theme(legend.position = "bottom") +
  scale_colour_viridis_d() +
  scale_fill_viridis_d()
```


### Mean of errors

```{r}
list(
  VAR = pred_var,
  VHAR = pred_vhar,
  BVAR = pred_bvar,
  BVHAR1 = pred_bvhar_v1,
  BVHAR2 = pred_bvhar_v2
) %>% 
  kable_lossmean(y = y_test, format = kable_format)
```

### Each variable

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  plot_loss(y = y_test) +
  # scale_y_log10() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_colour_viridis_d()
```


```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  make_kable(y = y_test, error = "mse", format = kable_format)
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  plot_loss(y = y_test, type = "mae") +
  # scale_y_log10() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_colour_viridis_d()
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  make_kable(y = y_test, error = "mae", format = kable_format)
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  plot_loss(y = y_test, type = "mape") +
  # scale_y_log10() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_colour_viridis_d()
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  make_kable(y = y_test, error = "mae", format = kable_format)
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  make_kable(y = y_test, error = "mape", format = kable_format)
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  plot_loss(y = y_test, type = "mase") +
  # scale_y_log10() +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_colour_viridis_d()
```

```{r}
list(
  pred_var,
  pred_vhar,
  pred_bvar,
  pred_bvhar_v1,
  pred_bvhar_v2
) %>% 
  make_kable(y = y_test, error = "mase", format = kable_format)
```


