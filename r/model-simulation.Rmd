...
```{r, message=FALSE}
# Train set
small_train <- read_csv("../data/processed/vharsim_small_train.csv")
# Evaluation set
small_test <- 
...
small_train %>% 
  mutate(train = TRUE) %>% 
  bind_rows(small_test %>% mutate(train = FALSE)) %>% 
  mutate(id = 1:n()) %>% 
...
```{r}
fit_var <- var_lm(small_train, var_lag, include_mean = FALSE)
...
```{r}
fit_vhar <- vhar_lm(small_train, include_mean = FALSE)
...
```{r}
(bvar_spec <- set_bvar(
  sigma = apply(small_train, 2, sd),
  lambda = .5,
  delta = rep(.1, ncol(small_train)),
  eps = 1e-04
))
```
...
(bvar_optim <- choose_bvar(
  bvar_spec, 
  lower = c(
    rep(.0001, ncol(small_train)), # sigma
    .01, # lambda
    rep(.01, ncol(small_train)) # delta
  ), 
  upper = c(
    rep(1, ncol(small_train)), # sigma
    2, # lambda
    rep(1, ncol(small_train)) # delta
  ), 
  y = small_train, 
  p = bvar_lag, 
  include_mean = FALSE
...
(bvhar_spec_vhar <- set_weight_bvhar(
  ...
  lambda = .5,
  eps = 1e-04,
  daily = rep(.1, ncol(small_train)),
  weekly = rep(.1, ncol(small_train)),
  monthly = rep(.1, ncol(small_train))
))
```
...
(bvhar_vhar_optim <- choose_bvhar(
  ...
  lower = c(
    rep(.0001, ncol(small_train)), # sigma
    .01, # lambda
    rep(.01, ncol(small_train)), # daily
    rep(.01, ncol(small_train)), # weekly
    rep(.01, ncol(small_train)) # monthly
  ), 
  upper = c(
    rep(1, ncol(small_train)), # sigma
    2, # lambda
    rep(1, ncol(small_train)), # daily
    rep(1, ncol(small_train)), # weekly
    rep(1, ncol(small_train)) # monthly
  ), 
  y = small_train, 
  include_mean = FALSE
))
...
list(
  ...
  make_kable(y = small_test, error = "mse", format = kable_format)
```
...
list(
  ...
  make_kable(y = small_test, error = "mae", format = kable_format)
```
...
list(
  ...
  make_kable(y = small_test, error = "mape", format = kable_format)
```
...
list(
  ...
  make_kable(y = small_test, error = "mase", format = kable_format)
```
...
list(
  ...
  kable_lossmean(y = small_test, format = kable_format)
```
...
# Evaluation set
medium_test <- read_csv("../data/processed/vharsim_medium_test.csv")
```
