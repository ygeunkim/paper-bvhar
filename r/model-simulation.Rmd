...
```{r}
cv_var_1 <- forecast_roll(fit_var, 5, y_test)
cv_vhar_1 <- forecast_roll(fit_vhar, 5, y_test)
cv_bvar_1 <- forecast_roll(fit_bvar, 5, y_test)
cv_bvhar_var_1 <- forecast_roll(fit_bvhar_var, 5, y_test)
cv_bvhar_vhar_1 <- forecast_roll(fit_bvhar_vhar, 5, y_test)
```
...
list(
  ...
  plot_loss(y_test, mean_line = TRUE, mean_param = list(alpha = .5))
```
