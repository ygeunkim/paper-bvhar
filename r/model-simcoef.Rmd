...
```{r}
get_rmfe(mod_list = cv_large_list, y = y_large_test, benchmark_id = 1)
```

```{r}
get_rmafetex(
  list(cv_small_list, cv_medium_list, cv_small_list), 
  y_list = list(y_small_test, y_medium_test, y_large_test), 
  ahead_list = c("$h = 1$", "$h = 5$", "$h = 20$"),
  benchmark_id = 1,
  caption = "Out-of-Sample Forecasting for Simulation", 
  label = "simresult"
) %>% 
  writeLines()
```
