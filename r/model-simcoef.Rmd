...
```{r medcvonefig}
cv_medium_1 %>% 
  gg_loss(y_medium_test, mean_line = TRUE, mean_param = list(alpha = .5)) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = -30, vjust = -1),
    legend.title = element_text(size = 8),
...
cv_medium_1 %>% 
  get_losstex(y_medium_test, "MEDIUM Simulation - 1-step ahead Rolling Window Forecasting Loss", label = "smallone") %>% 
  writeLines()
```

```{r medcvfivetab, comment=NULL, echo=FALSE}
cv_medium_5 %>% 
  get_losstex(y_medium_test, "MEDIUM Simulation - 5-step ahead Rolling Window Forecasting Loss", label = "smallfive") %>% 
  writeLines()
```

```{r medcvtwentytab, comment=NULL, echo=FALSE}
cv_medium_20 %>% 
  get_losstex(y_medium_test, "MEDIUM Simulation - 20-step ahead Rolling Window Forecasting Loss", label = "smalltwenty") %>% 
  writeLines()
```
...
```{r largecvonefig}
cv_large_1 %>% 
  gg_loss(y_large_test, mean_line = TRUE, mean_param = list(alpha = .5)) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = -30, vjust = -1),
    legend.title = element_text(size = 8),
