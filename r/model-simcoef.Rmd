...
```{r smallbvarsetting, comment=NULL, echo=FALSE}
n_small <- 3
...
```{r medbvarsetting, comment=NULL, echo=FALSE}
n_medium <- 9
...
```{r largebvarsetting, comment=NULL, echo=FALSE}
n_large <- 12
...
```{r}
y_small_train <- read_csv("../data/processed/bvarsim_small_train.csv")
y_small_test <- read_csv("../data/processed/bvarsim_small_test.csv")
```
...
```{r}
y_medium_train <- read_csv("../data/processed/bvarsim_medium_train.csv")
y_medium_test <- read_csv("../data/processed/bvarsim_medium_test.csv")
```
...
```{r}
y_large_train <- read_csv("../data/processed/bvarsim_large_train.csv")
y_large_test <- read_csv("../data/processed/bvarsim_large_test.csv")
```
...
list(
  ...
  report_hyperparam(caption = "MEDIUM Simulation - Hyperparameter Lists", label = "hyperparammed") %>% 
  writeLines()
```
...
list(
  ...
  report_hyperparam(caption = "LARGE Simulation - Hyperparameter Lists", label = "hyperparamlarge") %>% 
  writeLines()
```
...
```{r smallcvonefig}
cv_small_1 %>% 
  gg_loss(y_small_test, mean_line = TRUE, mean_param = list(alpha = .5), viridis = TRUE, show.legend = FALSE) +
  theme_minimal() +
  theme(
...
```{r medcvonefig}
cv_medium_1 %>% 
  gg_loss(y_medium_test, mean_line = TRUE, mean_param = list(alpha = .5), viridis = TRUE, show.legend = FALSE) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = -30, vjust = -1),
    legend.title = element_text(size = 8),
...
```{r largecvonefig}
cv_large_1 %>% 
  gg_loss(y_large_test, mean_line = TRUE, mean_param = list(alpha = .5), viridis = TRUE, show.legend = FALSE) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = -30, vjust = -1),
    legend.title = element_text(size = 8),
...
<!-- tibble( -->
<!--   y = 5:1, -->
<!--   x = c("BVAR_Minnesota", "BVHAR_MN_VAR", "BVHAR_MN_VHAR", "VAR", "VHAR") -->
<!-- ) %>%  -->
<!--   ggplot() + -->
<!--   geom_hline(aes(yintercept = y, color = x)) + -->
<!--   geom_text( -->
<!--     aes(x = -1, y = y, label = x) -->
<!--   ) + -->
<!--   theme_minimal() + -->
...
tibble(
  ...
  mutate(mod_label = ifelse(x == max(x), mod, NA_character_)) %>% 
  ggplot(aes(y = y)) +
...
<!-- pred_small <-  -->
<!--   mod_small_list %>%  -->
<!--   lapply(predict, n_ahead = h_step) -->
<!-- ``` -->

<!-- ```{r smallpreddensity} -->
<!-- autoplot(pred_small[[1]], x_cut = 980, ci_alpha = .7, type = "wrap") + -->
<!--   autolayer(pred_small[[2]], ci_alpha = .6) + -->
<!--   # autolayer(pred_small[[3]], ci_alpha = .4) + -->
<!--   # autolayer(pred_small[[4]], ci_alpha = .2) + -->
<!--   # autolayer(pred_small[[5]], ci_alpha = .1) -->
<!--   geom_eval(data = y_small_train[1:h_step,], num_train = nrow(y_small_train), alpha = .5) -->
<!-- ``` -->
...
<!-- ```{r medpreddensity} -->
<!-- autoplot(pred_medium[[1]], x_cut = 980, ci_alpha = .7, type = "wrap") + -->
<!--   autolayer(pred_medium[[2]], ci_alpha = .6) + -->
<!--   autolayer(pred_medium[[3]], ci_alpha = .4) + -->
<!--   autolayer(pred_medium[[4]], ci_alpha = .2) + -->
<!--   # autolayer(pred_medium[[5]], ci_alpha = .1) + -->
<!--   geom_eval(data = y_medium_test[1:h_step,], num_train = nrow(y_medium_train), alpha = .5) -->
<!-- ``` -->
...
<!-- ```{r largepreddensity} -->
<!-- autoplot(pred_large[[1]], x_cut = 980, ci_alpha = .7, type = "wrap") + -->
<!--   autolayer(pred_large[[2]], ci_alpha = .6) + -->
<!--   # autolayer(pred_large[[3]], ci_alpha = .4) + -->
<!--   # autolayer(pred_large[[4]], ci_alpha = .2) + -->
<!--   # autolayer(pred_medium[[5]], ci_alpha = .1) + -->
<!--   geom_eval(data = y_large_test[1:h_step,], num_train = nrow(y_large_train), alpha = .5) -->
<!-- ``` -->
