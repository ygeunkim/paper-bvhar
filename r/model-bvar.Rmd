...
bvar_small_spec %>% 
  kable_hyperparam(caption = "\\label{tab:smalltruespec}BVAR(5) and BVHAR Coefficient Simulation Setting - SMALL") %>% 
  writeLines()
```
...
bvar_small_spec %>% 
  kable_hyperparam(caption = "\\label{tab:medtruespec}BVAR(5) and BVHAR Coefficient Simulation Setting - MEDIUM") %>% 
  writeLines()
```
...
bvar_large_spec %>% 
  kable_hyperparam(caption = "\\label{tab:largetruespec}BVAR(5) and BVHAR Coefficient Simulation Setting - LARGE") %>% 
  writeLines()
```
...
get_rmafetex(
  ...
  benchmark_id = 1,
  caption = "Out-of-Sample Forecasting - Simulation for BVAR", 
  label = "simaveresult",
  header_angle = 15
) %>% 
  writeLines()
